# Random State Generator v2.0 (updated 2025-09-01)
...
Ctrl+R - Очистить данные"""
    ...
    def validate_inputs(self) -> tuple:
        ...
        try:
            ...
            return dist_type, param_values, sample_size
    ...
    def generate_distribution(self) -> None:
        ...
        try:
            # Validate inputs
            dist_type, param_values, sample_size = self.validate_inputs()
            ...
            values = self._generate_values(dist_type, param_values, sample_size)
    ...
    def _generate_values(self, dist_type: str, param_values: Dict[str, float],
                         sample_size: int) -> Optional[np.ndarray]:
        """Generate values for the specified distribution type."""
        try:
            if dist_type == "Нормальное":
                values = np.random.normal(param_values["mean"], param_values["std"], sample_size)
                ...
                values = np.random.lognormal(mu, sigma, sample_size)
            elif dist_type == "Равномерное":
                values = np.random.uniform(param_values["min"], param_values["max"], sample_size)
            elif dist_type == "Треугольное":
                values = np.random.triangular(param_values["min"], param_values["mode"], param_values["max"],
                                              sample_size)
            elif dist_type == "Бета":
                values = np.random.beta(param_values["alpha"], param_values["beta"], sample_size)
            elif dist_type == "Гамма":
                values = np.random.gamma(param_values["shape"], param_values["scale"], sample_size)
            elif dist_type == "Парето":
                values = np.random.pareto(param_values["shape"], sample_size) * param_values["scale"]
            elif dist_type == "Стьюдента t":
                values = np.random.standard_t(param_values["df"], sample_size)
            else:
                return None
