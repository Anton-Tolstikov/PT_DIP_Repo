...
def load_data(
	...
	# Map user-selected column names to canonical ones if needed
...
def sample_pp_parameters(
	...
	required_cols: Tuple[str, str, str, str] = ("Kp", "Kg", "Ps", "Pt"),
) -> pd.DataFrame:
	...
	return pd.DataFrame({
		...
		"Kg": rng.choice(coerce_float(pp_df["Kg"].dropna()).values, size=df_len, replace=True),
		"Ps": rng.choice(coerce_float(pp_df["Ps"].dropna()).values, size=df_len, replace=True) / 1000.0,
		"Pt": rng.choice(coerce_float(pp_df["Pt"].dropna()).values, size=df_len, replace=True),
	})
...
def calculate_parameters(df: pd.DataFrame, pp_df: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
	...
	df["Qgas"] = df["Area"] * df["Hnn"] * df["Kp"] * df["Kg"] * df["Ps"] * df["Pt"]
	return df
...
def build_percentile_table(df: pd.DataFrame, percentiles: List[int], tracked_params: Optional[List[str]] = None) -> pd.DataFrame:
	...
	required_for_calc = ["Area", "Hnn", "Kp", "Kg", "Ps", "Pt"]
	default_tracked = ["Area", "Hnn", "Kp", "Kg", "Ps", "Pt", "Qgas", "Qgas_GCoS"]
	...
	# Ensure Qgas_Calc is available for selecting best rows, even if user didn't ask to track inputs
	if all(col in df.columns for col in required_for_calc):
		...
		table["Qgas_Calc"] = arr["Area"] * arr["Hnn"] * arr["Kp"] * arr["Kg"] * arr["Ps"] * arr["Pt"]
	else:
		table["Qgas_Calc"] = np.nan
...
def assign_target_percentiles(df: pd.DataFrame, percentiles_check: List[int]) -> pd.DataFrame:
	df = df.copy()
	qgas_percentiles = np.percentile(df["Qgas"].dropna(), percentiles_check)
	labels = [f"P{100 - int(p)}" for p in percentiles_check]

	def nearest_label(x: float) -> str:
		idx = int(np.argmin(np.abs(qgas_percentiles - x)))
		return labels[idx]

	df["Target_Qgas"] = df["Qgas"].apply(nearest_label)
	return df
...
def get_closest_realizations(df: pd.DataFrame, percentiles: List[int], k: int = 3) -> pd.DataFrame:
	...
	targets = df["Qgas"].quantile(quant_levels).to_numpy()
	labels = [f"P{100 - p}" for p in percentiles]

	parts = []
	for label, target in zip(labels, targets):
		deviations = (df["Qgas"] - target).abs()
		closest = df.loc[deviations.nsmallest(k).index].copy()
		closest["Target_Qgas"] = label
...
def get_closest_by_area_hnn(
	...
	qgas_percentiles: List[int],
	k: int = 3,
) -> pd.DataFrame:
	df = df.copy()
	results = []

	qgas_real_targets = [np.percentile(df["Qgas"].dropna(), p) for p in qgas_percentiles]
	tmp = table_main.copy()

	for p_label, q_target in zip(qgas_percentiles, qgas_real_targets):
		tmp["Diff"] = (tmp["Qgas_Calc"] - q_target).abs()
		...
		closest_df["Target_Qgas"] = f"P{100 - int(p_label)}"
...
def get_best_percentile_rows(
	...
	rows = []
	q_targets = [np.percentile(df["Qgas"].dropna(), p) for p in target_percentiles]
	has_gcos = "Qgas_GCoS" in df.columns
	q_gcos_targets = [np.percentile(df["Qgas_GCoS"].dropna(), p) for p in target_percentiles] if has_gcos else [np.nan] * len(target_percentiles)
	...
	for p, q_target, q_target_gcos in zip(target_percentiles, q_targets, q_gcos_targets):
		...
		best_row["Qgas"] = q_target
		best_row["Qgas_GCoS"] = q_target_gcos
...
def write_excel_report(
	...
	with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
		...
		ws_final = workbook.add_worksheet("Кейс Газ РБ")
		writer.sheets["Кейс Газ РБ"] = ws_final

		headers = [
			"Пласт", "Категория", "Вариант", "F,тыс. м2", "Hэфф.гн, м",
			"Кп, д.ед.", "Кг, д.ед.", "Поправка на свойства газа, д.ед",
			"Поправка на температуру, д.ед", "НГЗ/НГР газа, млн.м3", "GCoS", "НГЗ/НГР газа в случае ГУ, млн.м3",
		]
		...
		for i, row in best_rows.iterrows():
			...
			table_final.loc[i] = [
				...
				row.get("Kp", np.nan), row.get("Kg", np.nan),
				row.get("Ps", np.nan), row.get("Pt", np.nan),
				row.get("Qgas", np.nan), gcos_value, row.get("Qgas_GCoS", np.nan),
			]
		...
		cols = ["Percentile"]
		headers = ["Процентиль"]
		if "Qgas" in table_main.columns:
			cols.append("Qgas")
			headers.append("Qgas, млн.м3")
		if "Qgas_GCoS" in table_main.columns:
			cols.append("Qgas_GCoS")
			headers.append("Qgas с учетом GCoS, млн.м3")
		...
		if tracked_params:
			...
			parameters = [c for c in ["Hnn", "Area", "Qgas", "Kp", "Kg", "Ps", "Pt"] if c in df.columns]
		...
		qgas_series = df["Qgas"].dropna()
		hist, edges = np.histogram(qgas_series, bins=bins)

		qgas_chart = workbook.add_chart({"type": "column"})
		...
		ws_final.write(row_offset, 0, f"Гистограмма {category} (млн.м³)", header_format)
		...
		qgas_chart.add_series({
			"categories": ["Кейс Газ РБ", row_offset + 1, 0, row_offset + len(hist), 0],
			"values": ["Кейс Газ РБ", row_offset + 1, 1, row_offset + len(hist), 1],
			"fill": {"color": "#4F81BD"},
		})
		qgas_chart.set_title({"name": f"{category} газа", "name_font": {"name": "Roboto", "size": 10, "bold": True}})
		qgas_chart.set_x_axis({
			"name": f"{category} газа, млн.м3",
			"name_font": {"name": "Roboto", "size": 6, "bold": False},
			"num_font": {"name": "Roboto", "size": 6},
			"major_gridlines": {"visible": True},
			"label_position": "low",
		})
		qgas_chart.set_y_axis({
			"name": "Количество",
			"name_font": {"name": "Roboto", "size": 6, "bold": False},
			"num_font": {"name": "Roboto", "size": 6},
			"major_gridlines": {"visible": True},
		...
		qgas_chart.set_legend({"none": True})
		qgas_chart.set_size({"width": 300, "height": 400})
		ws_final.insert_chart(row_offset, 3, qgas_chart)

		if "Qgas_GCoS" in df.columns and df["Qgas_GCoS"].notna().any():
			hist_gcos, edges_gcos = np.histogram(df["Qgas_GCoS"].dropna(), bins=bins)
			ws_final.write(row_offset, 6, f"Гистограмма {category} с учетом GCoS (млн.м³)", header_format)
			...
			qgas_gcos_chart = workbook.add_chart({"type": "column"})
			qgas_gcos_chart.add_series({
				"categories": ["Кейс Газ РБ", row_offset + 1, 6, row_offset + len(hist_gcos), 6],
				"values": ["Кейс Газ РБ", row_offset + 1, 7, row_offset + len(hist_gcos), 7],
				"fill": {"color": "#C0504D"},
			})
			qgas_gcos_chart.set_title({"name": f"{category} газа с учетом GCoS", "name_font": {"name": "Roboto", "size": 10}})
			qgas_gcos_chart.set_x_axis({
				"name": f"{category} газа с учетом GCoS, млн.м3",
				"name_font": {"name": "Roboto", "size": 6, "bold": False},
				"num_font": {"name": "Roboto", "size": 6},
				"major_gridlines": {"visible": True},
				"label_position": "low",
			})
			qgas_gcos_chart.set_y_axis({
				"name": "Количество",
				"name_font": {"name": "Roboto", "size": 6, "bold": False},
				"num_font": {"name": "Roboto", "size": 6},
				"major_gridlines": {"visible": True},
			...
			qgas_gcos_chart.set_size({"width": 300, "height": 400})
			qgas_gcos_chart.set_legend({"none": True})
			ws_final.insert_chart(row_offset, 9, qgas_gcos_chart)
...
def run_pipeline(
	...
	for filename in all_files:
		...
		try:
			...
			if gcos_value is not None:
				...
				df["Qgas_GCoS"] = np.where(success_coef <= gcos_value, df["Qgas"], 0.0)
			...
			for _, row in best_rows.iterrows():
				summary_rows.append({
					...
					"Hэфф.гн, м": row.get("Hnn", np.nan),
					"Кп, д.ед.": row.get("Kp", np.nan),
					"Кг, д.ед.": row.get("Kg", np.nan),
					"Поправка на свойства газа, д.ед": row.get("Ps", np.nan),
					"Поправка на температуру, д.ед": row.get("Pt", np.nan),
					"НГЗ/НГР газа, млн.м3": row.get("Qgas", np.nan),
					"GCoS": gcos_value,
					"НГЗ/НГР газа в случае ГУ, млн.м3": row.get("Qgas_GCoS", np.nan),
				})

			output_path = os.path.join(output_folder, f"{object_name}_report_gas.xlsx")
			write_excel_report(
	...
	if summary_rows:
		...
		summary_path = os.path.join(output_folder, "Сводная_таблица_оценок_Кейс_Газ.xlsx")
...
SETTINGS_PATH = Path.home() / ".gas_case_gui_settings.json"


class GasCaseGUI(tk.Tk):
	def __init__(self):
		super().__init__()
		self.title("Gas Case Report Builder")
	...
	def _build_vars(self):
		...
		self.var_tracked_params = tk.StringVar(value="Area,Hnn,Kp,Kg,Ps,Pt,Qgas,Qgas_GCoS")
	...
	def _load_settings(self):
		try:
			if SETTINGS_PATH.exists():
				...
				self.var_tracked_params.set(data.get("tracked_params", "Area,Hnn,Kp,Kg,Ps,Pt,Qgas,Qgas_GCoS"))
