...
#Constants
DEFAULT_FONTS = ["Tahoma", "Roboto", "Arial", "Times New Roman", "Calibri"]
DEFAULT_FONT = "Roboto"
...
# Distribution types and their params
DISTRIBUTION_TYPES = {
    "Нормальное": ["Среднее", "STD"],
    "Лог-нормальное": ["Среднее", "STD"],
    "Равномерное": ["Минимум", "Максимум"],
    "Треугольное": ["Минимум", "Максимум", "Мода"],
    "Бета": ["Альфа", "Бэта"],
    "Гамма": ["Форма", "Масштаб"],
    "Парето": ["Форма", "Масштаб"],
    "Стьюдента t": ["Степени свободы"]
}

#Setting font. Getting default if it's not avalible
try:
...
THEME_PETROTRACE = ThemeConfig(
    ...
    mpl_axes_face="#FFFFFF",# белый фон осей
    mpl_grid="#D4E8D4",     # светло-зеленая сетка
    hist_color="#228B22",   # forest green гистограмма
    font_family="Tahoma",
)
...
THEME_GPN = ThemeConfig(
    ...
    mpl_axes_face="#FFFFFF",# белый фон осей
    mpl_grid="#C3D9FF",     # более светлая синяя сетка
    hist_color="#0088DA",   # синий Газпрома для гистограммы
    font_family="Roboto",
)

THEME_CUSTOM_DEFAULT = ThemeConfig(
    name="Пользовательская",
    bg="#FFFFFF",           # белый фон
    fg="#000000",           # черный текст
    accent="#888888",       # серый акцент
    frame_bg="#F0F0F0",     # светло-серый фон фреймов
    entry_bg="#FFFFFF",     # белый фон полей ввода
    entry_fg="#000000",     # черный текст полей ввода
    button_bg="#A0A0A0",    # светло-серый кнопки
    button_fg="#FFFFFF",    # белый текст кнопок
    mpl_face="#FFFFFF",     # белый фон графика
    mpl_axes_face="#F8F8F8",# очень светло-серый фон осей
    mpl_grid="#CCCCCC",     # светло-серая сетка
    hist_color="#808080",   # серый гистограмма
    font_family="Arial",
)

THEMES: Dict[str, ThemeConfig] = {
    "GPN": THEME_GPN,
    "PetroTrace": THEME_PETROTRACE,
    "Пользовательская": THEME_CUSTOM_DEFAULT,
}
...
class DistributionApp:
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Random_Genrator_v2.0")
        ...
        self.theme_var = tk.StringVar(value="PetroTrace")
        self.custom_theme: ThemeConfig = THEME_CUSTOM_DEFAULT
    ...
    def setup_ui(self) -> None:
        try:
            ...
            messagebox.showerror("Ошибка", f"Ошибка формирования интерфейса: {e}")

    def on_close(self) -> None:
        #Application closing control
        try:
            if self.fig:
                plt.close(self.fig)
            plt.close('all')
    ...
    def _on_left_mousewheel(self, event):
        #Scrolling the left panel with mouse wheel when bound on it
        try:
    ...
    def setup_params_list(self) -> None:
        #Setup rigth part of interface
        try:
            ttk.Label(self.right_frame, text="Текущая история генератора",
            ...
            ttk.Radiobutton(theme_frame, text="Пользовательская", variable=self.theme_var,
                            value="Пользовательская", command=self.apply_theme).pack(anchor=tk.W)


            # Add keyboard shortcuts info
            shortcuts_frame = ttk.LabelFrame(self.right_frame, text="Список горячих клавиш", padding=5)
...
"""
    ...
    def setup_left_frame(self) -> None:
        ...
        try:
            ...
            excel_frame = ttk.LabelFrame(self.left_frame, text="Загрузка данных Excel для анализа", padding=10)
            ...
            ttk.Button(excel_frame, text="Создать гистограмму из Excel",
                       command=self.create_histogram_from_excel).pack(fill=tk.X, pady=2)

            # Distribution parameters frame
            dist_frame = ttk.LabelFrame(self.left_frame, text="Параметры генератора", padding=10)
            ...
            ttk.Label(dist_frame, text="Название параметра (Для диаграммы):").pack()
            ...
            ttk.Label(dist_frame, text="Единицы измерения (Для диаграммы):").pack()
            self.unit_entry = ttk.Entry(dist_frame)
            self.unit_entry.insert(0, DEFAULT_UNIT)
            self.unit_entry.pack(fill=tk.X)
            ttk.Button(dist_frame, text="Сгенерировать",
                       command=self.generate_distribution).pack(fill=tk.X, pady=2)

            # Data management frame
            data_frame = ttk.LabelFrame(self.left_frame, text="Константы директории", padding=10)
            ...
            ttk.Button(data_frame, text="Очистить файл с линиями",
                       command=self.clear_lines_file).pack(fill=tk.X, pady=5)

            # Actions frame
            action_frame = ttk.LabelFrame(self.left_frame, text="Действия", padding=10)
            action_frame.pack(fill=tk.X, pady=5)

            ttk.Button(action_frame, text="Сохранить текущую гистаграмму",
                       command=self.save_plot).pack(fill=tk.X, pady=2)
            ttk.Button(action_frame, text="Сохранить данные генераций",
                       command=self.save_data).pack(fill=tk.X, pady=2)
            ttk.Label(action_frame, text="").pack(pady=5)
            ...
            ttk.Label(action_frame, text="").pack(pady=10)
            ttk.Button(action_frame, text="Очистить все данные и кэш",
    ...
    def setup_parameter_inputs(self) -> None:
        #Setup defaults params for every distributiuns
        try:
    ...
    def on_sheet_selected(self, event=None) -> None:
        #Handle sheet selection function
        try:
    ...
    def load_excel_file(self) -> None:
        #Loading excel for distribution data
        try:
    ...
    def create_histogram_from_excel(self) -> None:
        #Creating histogram from excel data
        try:
    ...
    def choose_hist_color(self) -> None:
        #Color choosing
        try:
            color = colorchooser.askcolor(color=self.hist_color, title="Выбрать цвет гистограммы")
    ...
    def apply_theme(self) -> None:
        ...
        try:
            ...
            theme = THEMES.get(theme_name, THEME_GPN)

            # Apply Tkinter/ttk colors (best effort)
            style = ttk.Style()
            ...
            rcParams['figure.facecolor'] = theme.mpl_face
            rcParams['axes.facecolor'] = theme.mpl_axes_face
            rcParams['axes.edgecolor'] = theme.fg
            rcParams['axes.labelcolor'] = theme.fg
            rcParams['text.color'] = theme.fg
            rcParams['xtick.color'] = theme.fg
            rcParams['ytick.color'] = theme.fg
            rcParams['grid.color'] = theme.mpl_grid
            rcParams['grid.alpha'] = 0.3

            # Apply font
            self.current_font = theme.font_family
            rcParams['font.family'] = theme.font_family
            self.font_combobox.set(theme.font_family)

            # Apply histogram color
            self.hist_color = theme.hist_color

            # Update current plot if exists
            if self.current_values is not None:
                param_name = self.param_name_entry.get() or DEFAULT_PARAM_NAME
                unit = self.unit_entry.get()
                self._update_plot(param_name, unit, self.current_values)
    ...
    def validate_inputs(self) -> tuple:
        ...
        try:
            ...
            if dist_type == "Треугольное":
                if not (param_values["Минимум"] < param_values["Мода"] < param_values["Максимум"]):
                    raise ValueError("Для треугольного распределения требуется: Минимум < Мода < Максимум")
    ...
    def generate_distribution(self) -> None:
        ...
        try:
            ...
            if sample_size > 10000:
                messagebox.showinfo("Информация", f"Генерация {sample_size:,} значений может занять значительное")
        ...
        try:
            def _generate(size: int) -> np.ndarray:
                if dist_type == "Нормальное":
                    return np.random.normal(param_values["Среднее"], param_values["STD"], size)
                elif dist_type == "Лог-нормальное":
                    if param_values["Среднее"] <= 0:
                        raise ValueError("Среднее значение должно быть положительным для лог-нормального распределения")
                    mu = np.log(param_values["Среднее"] / np.sqrt(1 + (param_values["STD"] / param_values["Среднее"]) ** 2))
                    sigma = np.sqrt(np.log(1 + (param_values["STD"] / param_values["Среднее"]) ** 2))
                    return np.random.lognormal(mu, sigma, size)
                elif dist_type == "Равномерное":
                    return np.random.uniform(param_values["Минимум"], param_values["Максимум"], size)
                elif dist_type == "Треугольное":
                    return np.random.triangular(param_values["Минимум"], param_values["Мода"], param_values["Максимум"], size)
                elif dist_type == "Бета":
                    return np.random.beta(param_values["Альфа"], param_values["Бэта"], size)
                elif dist_type == "Гамма":
                    return np.random.gamma(param_values["Форма"], param_values["Масштаб"], size)
                elif dist_type == "Парето":
                    return np.random.pareto(param_values["Форма"], size) * param_values["Масштаб"]
                elif dist_type == "Стьюдента t":
                    return np.random.standard_t(param_values["Степени свободы"], size)
                else:
                    return np.array([])
    ...
    def _add_reference_lines(self) -> None:
        #Adding reference lines on plot
        try:
    ...
    def _show_distribution_analysis(self, results: List[Dict], original_dist_type: str = None,
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                text_widget.insert(tk.END, f"{i}. {result['Распределение']}\n")
                ...
                param_labels = self._get_parameter_labels(result['Распределение'], result['Параметры'])
    ...
    def clear_lines_file(self) -> None:
        #Clear lines reference list
        try:
            # Clear the data DataFrame
            self.data = None

            # Reset status labels
            self.excel_label.config(text="Файл не загружен", foreground="gray")

            # Clear reference lines from plot if they exist
            if self.ax:
                # Remove all vertical lines (reference lines)
                ...
                for text in self.ax.texts:
                    if text.get_color() == 'red':
                        text.remove()

                self.canvas.draw()

            messagebox.showinfo("Очистка", "Файл с линиями успешно очищен")
            logger.info("Line file data cleared successfully")

        except Exception as e:
            logger.error(f"Failed to clear line file data: {e}")
            messagebox.showerror("Ошибка", f"Не удалось очистить файл с линиями:\n{e}")
def main():
    #main loop starting
    try:
        root = tk.Tk()
        ...
        messagebox.showerror("Критическая ошибка", f"Не удается запустить приложение:\n{e}")
